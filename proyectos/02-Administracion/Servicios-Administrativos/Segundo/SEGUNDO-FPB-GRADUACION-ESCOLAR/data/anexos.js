// Anexos del Proyecto
window.sectionsContent.anexos = `
<div class="callout callout-info">
    <div class="callout-title">
        <span>📎</span>
        <span>Anexos Técnicos</span>
    </div>
    <p class="mb-0">Modelos de documentos, plantillas y materiales complementarios para el proyecto.</p>
</div>

## Anexo 1: Modelo de Autorización de Participación 📝

```
AUTORIZACIÓN PARA PARTICIPACIÓN EN ACTO DE GRADUACIÓN 2026
IES SERRA PERENXISA

D./Dña. _________________________________________________________
Con DNI/NIE nº _________________________________________________
En calidad de: ⬜ Padre ⬜ Madre ⬜ Tutor/a legal

De: ____________________________________________________________
Alumno/a de: ⬜ 4º ESO ⬜ FPB Servicios Administrativos
Grupo: _______

AUTORIZO:

□ Que mi hijo/a participe en el acto de graduación escolar 2026
□ Que aparezca en las fotografías y grabaciones del evento
□ Que las imágenes sean utilizadas para:
  ⬜ Archivo familiar (solo uso privado)
  ⬜ Web y redes sociales del centro
  ⬜ Publicaciones y memoria del centro

Datos de contacto:
• Teléfono: _________________________
• Email: ___________________________
• Dirección: _______________________
  ___________________________________

En __________________, a ____ de _______________ de 2026


Firma del padre/madre/tutor:
_____________________________
```

---

## Anexo 2: Modelo de Autorización de Uso de Imagen 📸

```
CONSENTIMIENTO PARA EL USO DE IMAGEN
GRADUACIÓN 2026 - IES SERRA PERENXISA

De conformidad con el Reglamento General de Protección de Datos (RGPD),

D./Dña. _________________________________________________________
Con DNI/NIE nº _________________________________________________

Como padre/madre/tutor legal de: ________________________________
Alumno/a de: __________________________________________________

DECLARO:

Que he sido informado/a de que durante el acto de graduación se 
realizarán captaciones de imagen (fotografías y/o vídeo) en las 
que podrá aparecer mi hijo/a.

Que se me informa de que las finalidades del tratamiento son:
1. Archivo familiar y recuerdo personal
2. Publicación en la web del centro (opcional)
3. Publicación en redes sociales del centro (opcional)
4. Memoria anual del centro (opcional)

Que los derechos que me asisten son:
• Acceder a las imágenes
• Solicitar su rectificación o supresión
• Oponerme a su tratamiento
• Solicitar la limitación del tratamiento

Que este consentimiento es VOLUNTARIO y puedo REVOCARLO en 
cualquier momento.

OPCIONES (marcar las que autoricen):

□ AUTORIZO la publicación en la web del centro
□ AUTORIZO la publicación en redes sociales del centro
□ AUTORIZO el uso en publicaciones y memoria del centro
□ NO AUTORIZO ningún uso público (solo archivo familiar)

En __________________, a ____ de _______________ de 2026


Firma: _______________________________

Nombre y apellidos: ___________________
Teléfono de contacto: _________________
Email: _______________________________
```

---

## Anexo 3: Modelo de Confirmación de Asistencia (RSVP) ✉️

```
CONFIRMACIÓN DE ASISTENCIA - GRADUACIÓN 2026

Datos del Graduando/a:
• Nombre: ___________________________________
• Curso: ____________________________________
• Grupo: ____________________________________

Datos del Familiar:
• Nombre: ___________________________________
• Parentesco: ⬜ Padre ⬜ Madre ⬜ Otro: _______
• Teléfono: _________________________________
• Email: ____________________________________

Asistencia al Acto:
• ¿Asistirá el graduando/a? ⬜ Sí ⬜ No
• Número de familiares asistentes: ____________
  (máximo recomendado: 4 por graduando)

Observaciones:
• ¿Necesita accesibilidad especial? ⬜ Sí ⬜ No
  En caso afirmativo, especifique: ____________
  ___________________________________________

• ¿Alguna alergia o intolerancia alimentaria?
  ⬜ Sí ⬜ No
  En caso afirmativo, especifique: ___________
  ___________________________________________

Fecha límite de entrega: ____ de mayo de 2026

Formas de envío:
□ Entregar en Secretaría del Centro
□ Enviar por email a: graduacion2026@iesserraperenxisa.es
□ Enviar por WhatsApp al: [número del equipo organizador]

¡Gracias por su colaboración!
```

---

## Anexo 4: Modelo de Presupuesto para Familias 💰

```
PRESUPUESTO GRADUACIÓN 2026
IES SERRA PERENXISA

CONCEPTO                     IMPORTE      CANTIDAD    TOTAL
─────────────────────────────────────────────────────────
Aportación por graduando     15,00€         1         15,00€
  (incluye organización, decoración, 
   programa del acto, brindis)

Entradas adicionales          5,00€         __         __,__€
  (para familiares más allá de los 4 incluidos)

Birrete y estola (opcional)   8,00€         __         __,__€
  (talla: ___)

Diploma adicional (opcional)  3,00€         __         __,__€

Fotos digitales (opcional)   10,00€         __         __,__€
  (pack de fotos profesionales del evento)

─────────────────────────────────────────────────────────
                              TOTAL A PAGAR: _________€
─────────────────────────────────────────────────────────

FORMAS DE PAGO:

□ Transferencia bancaria
  IBAN: ESXX XXXX XXXX XXXX XXXX XXXX
  Concepto: "Graduación2026_[Nombre alumno]"
  
□ Bizum
  Teléfono: [número del AMPA]
  Concepto: "Graduación2026_[Nombre alumno]"
  
□ Efectivo en Secretaría
  (se entregará recibo justificativo)

PLAZO DE PAGO: Antes del 15 de mayo de 2026

IMPORTANTE:
• El pago debe realizarse íntegro antes de la fecha indicada
• Las entradas adicionales son limitadas (sujeto a aforo)
• Los conceptos opcionales deben solicitarse al formalizar la inscripción

Para dudas o incidencias: graduacion2026@iesserraperenxisa.es
```

---

## Anexo 5: Modelo de Contrato con Proveedor 🤝

```
CONTRATO DE PRESTACIÓN DE SERVICIOS
GRADUACIÓN 2026 - IES SERRA PERENXISA

En __________________, a ____ de _______________ de 2026

REUNIDOS

DE UNA PARTE:
D./Dña. _______________________________________________________
En nombre de: ________________________________________________
Con NIF/CIF: _________________________________________________
Domicilio: ___________________________________________________
En adelante, EL PROVEEDOR

DE OTRA PARTE:
D./Dña. _______________________________________________________
En calidad de: Director/a del IES Serra Perenxisa
Con NIF: Q-4600000-X
Domicilio: C/ [Dirección del Centro], [Población]
En adelante, EL CENTRO

EXPONEN

Que EL CENTRO va a celebrar un acto de graduación el día ____ de 
junio de 2026, y necesita contratar los servicios de EL PROVEEDOR.

Que EL PROVEEDOR dispone de los medios necesarios para prestar 
dichos servicios.

Por ello, ambas partes acuerdan:

PRIMERO.- OBJETO DEL CONTRATO
EL PROVEEDOR se compromete a prestar los siguientes servicios:
_____________________________________________________________
_____________________________________________________________

SEGUNDO.- PRECIO Y FORMA DE PAGO
El precio total es de: __________€ (más/menos IVA)
Forma de pago: 
□ 50% al firmar, 50% tras el evento
□ 100% tras el evento (a 30 días)
□ Otro: __________________________________________________

TERCERO.- FECHA Y HORARIO
Fecha: ____ de junio de 2026
Horario: De ______:______ a ______:______ horas
Lugar: IES Serra Perenxisa, [Espacio concreto]

CUARTO.- OBLIGACIONES DEL PROVEEDOR
• Presentarse con 30 minutos de antelación
• Disponer de todo el material necesario
• Cumplir con la normativa de seguridad
• Emitir factura con IVA desglosado

QUINTO.- OBLIGACIONES DEL CENTRO
• Facilitar el acceso al espacio
• Proporcionar [lo que sea necesario]
• Abonar el precio convenido en el plazo acordado

SEXTO.- INCUMPLIMIENTO
En caso de incumplimiento por cualquiera de las partes, se 
podrá resolver el contrato y reclamar las daños y perjuicios 
causados.

Y en prueba de conformidad, firman el presente contrato por 
duplicado.


POR EL CENTRO:                          POR EL PROVEEDOR:

_______________________                 _______________________
El Director/a                            El Representante
```

---

## Anexo 6: Guión del Acto de Graduación 🎓

```
GUION DEL ACTO DE GRADUACIÓN 2026
IES SERRA PERENXISA
____ de junio de 2026 - 17:30 horas

─────────────────────────────────────────────────────────────

17:00 - 17:30 | ACOGIDA DE INVITADOS
─────────────────────────────────────────────────────────────
• Equipo de acogida en la entrada
• Entrega de programas del acto
• Indicación de asientos
• Música ambiental suave

─────────────────────────────────────────────────────────────
17:30 - 17:35 | INICIO DEL ACTO
─────────────────────────────────────────────────────────────
Maestro/a de Ceremonias: [Nombre alumno/a 1] y [Nombre alumno/a 2]

MC-1: "¡Buenas tardes a todos y todas!"
MC-2: "Bienvenidos al acto de graduación de la promoción 2026 
       del IES Serra Perenxisa."
MC-1: "Hoy es un día muy especial para nuestro alumnado de 
       4º de ESO y de 2º de FP Básica en Servicios Administrativos."
MC-2: "Comenzamos este acto que marca el fin de una etapa y 
       el comienzo de nuevos caminos."

─────────────────────────────────────────────────────────────
17:35 - 17:40 | HIMNO / IDENTIDAD DEL CENTRO
─────────────────────────────────────────────────────────────
• [Si hay himno del centro, se interpreta]
• [O minuto de silencio si procede]

─────────────────────────────────────────────────────────────
17:40 - 17:50 | DISCURSO DEL EQUIPO DIRECTIVO
─────────────────────────────────────────────────────────────
MC-1: "A continuación, invitamos a subir al escenario al 
       Director/a del centro, D./Dña. [Nombre]."

[Discurso del Director/a - 5 minutos aprox.]

─────────────────────────────────────────────────────────────
17:50 - 18:05 | DISCURSO DEL ALUMNADO
─────────────────────────────────────────────────────────────
MC-2: "Ahora es el momento de escuchar las palabras de quienes 
       hoy son los protagonistas."

[Discurso de representante del alumnado - 5 minutos aprox.]

[Opcional: Vídeo conmemorativo - 3 minutos]

─────────────────────────────────────────────────────────────
18:05 - 18:45 | ENTREGA DE DIPLOMAS
─────────────────────────────────────────────────────────────
MC-1: "Procedemos a la entrega de diplomas."
MC-2: "Iremos llamando al alumnado por orden alfabético."

LISTA DE GRADUANDOS:

4º ESO:
1. [Apellido, Nombre]
2. [Apellido, Nombre]
...

FPB Servicios Administrativos:
1. [Apellido, Nombre]
2. [Apellido, Nombre]
...

[Para cada graduando:]
• Se anuncia el nombre
• Sube al escenario
• Recibe el diploma del Director/a y Tutor/a
• Foto oficial (10 segundos)
• Baja del escenario

─────────────────────────────────────────────────────────────
18:45 - 18:55 | ACTUACIÓN ESPECIAL
─────────────────────────────────────────────────────────────
[Opcional: Actuación musical, baile, poesía...]

─────────────────────────────────────────────────────────────
18:55 - 19:00 | DESPEDIDA Y BRINDIS
─────────────────────────────────────────────────────────────
MC-1: "Ha llegado el momento de despedir este acto."
MC-2: "Pero antes, queremos invitarles a un brindis por los 
       graduandos."

MC-1 y MC-2 (al unísono): "¡Enhorabuena, promoción 2026!"

[Brindis con cava/sin alcohol]

─────────────────────────────────────────────────────────────
19:00 - 20:00 | FOTOS Y FELICITACIONES
─────────────────────────────────────────────────────────────
• Foto oficial de toda la promoción
• Fotos por grupos
• Tiempo para felicitaciones

─────────────────────────────────────────────────────────────
20:00 | FIN DEL ACTO
─────────────────────────────────────────────────────────────
```

---

## Anexo 7: Checklist de Organización 📋

```
CHECKLIST DE ORGANIZACIÓN - GRADUACIÓN 2026

MARZO (Semana 1-4)
□ Constitución del equipo organizador
□ Primera reunión y lluvia de ideas
□ Investigación de centros referentes
□ Solicitud formal a Dirección
□ Definición de imagen corporativa

ABRIL (Semana 5-8)
□ Diseño de invitaciones
□ Apertura de plazo de inscripciones
□ Búsqueda de proveedores
□ Petición de presupuestos
□ Reserva de espacio y fecha
□ Diseño del protocolo del acto

MAYO (Semana 9-12)
□ Confirmación de proveedores
□ Impresión de material
□ Recogida de autorizaciones
□ Compra de birretes y estolas
□ Confirmación definitiva de asistentes
□ Ensayo general

JUNIO (Semana 13-14)
□ Montaje del espacio
□ Celebración del evento
□ Desmontaje y limpieza
□ Agradecimientos
□ Cierre económico
□ Informe de evaluación
```

---

## Anexo 8: Plano del Espacio 🏛️

```
                    PLANO DEL SALÓN DE ACTOS
                    IES SERRA PERENXISA
                    
    ┌─────────────────────────────────────────────────────────┐
    │                      ESCENARIO                          │
    │                                                         │
    │    🎤        🪑🪑🪑        📷        🪑🪑🪑              │
    │                                                         │
    │              MESA DIRECTIVA                             │
    │                                                         │
    ├─────────────────────────────────────────────────────────┤
    │                                                         │
    │    FILAS 1-2: GRADUANDOS (80 sillas)                    │
    │    ┌───────────────────────────────────────────┐        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    └───────────────────────────────────────────┘        │
    │                                                         │
    │    FILAS 3-10: FAMILIAS (aprox. 200 sillas)             │
    │    ┌───────────────────────────────────────────┐        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    │ 🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑🪑 │        │
    │    │ ... (más filas) ...                        │        │
    │    └───────────────────────────────────────────┘        │
    │                                                         │
    │  🚪 ENTRADA    📷 ZONA FOTOS    ♿ ACCESIBLE    🚪 SALIDA│
    │                                                         │
    │              🍽️ ZONA BRINDIS (al fondo)                │
    └─────────────────────────────────────────────────────────┘
    
    LEYENDA:
    🪑 = Silla
    🎤 = Micrófono
    📷 = Cámara/Fotos
    🚪 = Puerta
    ♿ = Zona accesible
    🍽️ = Zona de brindis
```

---

<div class="callout callout-info">
    <div class="callout-title">
        <span>💡</span>
        <span>Uso de los Anexos</span>
    </div>
    <p class="mb-0">Estos modelos son <strong>plantillas base</strong> que debes adaptar a las necesidades concretas de tu centro educativo y evento. ¡Personalízalos!</p>
</div>
`;
